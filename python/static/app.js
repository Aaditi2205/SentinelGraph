...
async function updateBlastPreview() {
  ...
  if (state.incidentMode === "workbench") {
    try { await compileWorkbench(false); } catch (_) { /* status is rendered by compileWorkbench */ }
    return;
  }
...
function loadCopilotStatus(status) {
  ...
  el("generatorState").innerHTML = generator.configured ? "<i></i>Gemini live" : "<i></i>Cited local mode";
...
async function init() {
  bind();
  try {
    const [metricsResponse, incidentResponse, razorpayResponse, coverageResponse, copilotResponse] = await Promise.all([fetch("/api/metrics"), fetch("/api/incident"), fetch("/api/razorpay/status"), fetch("/api/razorpay/coverage"), fetch("/api/copilot/status")]);
    if (!metricsResponse.ok || !incidentResponse.ok) throw new Error("SentinelGraph artifacts are not ready");
    loadMetrics(await metricsResponse.json());
    state.originalIncident = await incidentResponse.json();
    loadIncident(state.originalIncident);
    ...
    loadCopilotStatus(await copilotResponse.json());
  } catch (error) {
    document.body.innerHTML = `<main class="fatal"><h1>SentinelGraph could not load</h1><p>${error.message}</p><code>python build_sentinelgraph.py --data-dir &lt;ieee-cis-folder&gt;</code></main>`;
  }
}
